...
{   
    ...
    {   
        ...
        block2 = matB[0].length / MAXTHREADS;
        ...
        for (i = 0; i < threads.length; i++) {
            if (i != threads.length - 1) {
                threads[i] = new Multiplicator(matA, matB, matC, (i * block), ((i + 1) * block), (i*block2), ((i + 1) * block));
            } else {
                threads[i] = new Multiplicator(matA, matB, matC, (i * block), matA.length, (i*block2), matB[0].length);
            }
        }
        // Start Timer
